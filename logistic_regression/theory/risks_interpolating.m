...
output_dir = 'logistic_regression_theory_regularized_large';
...
parfor gammai =1:sgam
    for lambdai = 1:sla
        ...
        % Save entire workspace, just in case
        %save(output_path);
    end
end
...
function ret = New_eval(x, eps, gamma, sigma, nmc, lambda)
    ...
    ret = zeros(6, 1);
    ret(2) = emZsigma + 2*lambda*vp;
    ret(1) = (emZ + 2*lambda*vo - sqrt(gval));
    ret(3) = (-emdelta *eps - mu);
    ret(4) = emtaul +r/2;
    ret(5) = emr - gr/(2*sqrt(gval)) * vo + taul/2;
    ret(6) = -vo*1/(2*sqrt(gval)) * gmu - delta;
end
